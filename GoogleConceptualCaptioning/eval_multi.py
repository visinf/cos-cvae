...
import os
import sys
import GoogleConceptualCaptioning.misc.utils as utils

from GoogleConceptualCaptioning.misc.div_utils import compute_div_n, compute_global_div_n

import sys
sys.path.append("coco-caption")
annFile = '/BS/apr/work/flickr/verse/coco/annotations/captions_val2014.json'
...
from GoogleConceptualCaptioning.cococaption.pycocoevalcap.tokenizer.ptbtokenizer import PTBTokenizer
from GoogleConceptualCaptioning.cococaption.pycocoevalcap.bleu.bleu import Bleu
...
def eval_spice_n(preds_n, model_id, split):
    ...
    cache_path_n = os.path.join('./', model_id + '_' + split + '_n_.json')
...
def eval_oracle(preds_n, model_id, split):
    ...
    print('capsById[capsById.keys() ', list(capsById.keys()))
    print('capsById[capsById.keys() ', capsById[list(capsById.keys())[0]])
    ...
    for img_id in capsById.keys():
        ...
        #print('capsById[img_id][0][scores] ', list(capsById[img_id][0].keys()))
        #print('img_id ',img_id)
        for metric in capsById[img_id][0]['scores'].keys():
            out['ImgToEval'][img_id][metric] = max([_['scores'][metric] for _ in capsById[img_id]])
            #print('img_id max -- ', max([_['scores'][metric] for _ in capsById[img_id]]))
...
def eval_div_stats(preds_n, model_id, split):
    ...
    n_caps_perimg = len(capsById[list(capsById.keys())[0]])#len(capsById[capsById.keys()[0]])
    print(n_caps_perimg)
