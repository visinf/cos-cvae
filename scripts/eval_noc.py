...
def check_if_novel( true_strs ):
	nc_flags = np.zeros((8,))
	idxs = []
	novel_classes = ['bottle ', 'bus ', 'couch ', 'microwave ', 'pizza ', 'racket ', 'suitcase ', 'zebra ']
	for j, _str in enumerate(true_strs):
		for i, nc in enumerate(novel_classes):
			if nc in (_str+' '):
				nc_flags[i] = 1
				idxs.append(j)
	return nc_flags, idxs

def load_dataset(data_path,vocab_path,coco_class,maxlength):
	image_data_path = str(data_path)+'/coco_test_2014_noc_adaptive_withclasses.h5'
	text_data_path = str(data_path)+'/annotations/captions_val2014.json'
	return COCO_Dataset(text_data_path,image_data_path,vocab_path,coco_class,None,maxlength)
...
if __name__== "__main__":
	...
	while True:
		...
		try:
			model = torch.load('./ckpts/model_checkpoint_noc_release.pt')
			...
			with torch.no_grad():
				...
				for j in range(batch_size):
					...
					for true_caption in seq[j,:]:
						...
						true_strs.append(' '.join(true_str))

					nc_flag_gt, _ = check_if_novel( true_strs )
					classes_gt.append(nc_flag_gt)

					all_test_sample_score_spice = []
					all_test_samples = []
					for test_sample_idx in range(beam_width*test_samples):
						...
						all_test_samples.append(curr_pred_str)

					nc_flag_pred, pred_idxs = check_if_novel( all_test_samples )
					classes_pred_100.append(nc_flag_pred)
		classes_gt = np.array(classes_gt)
		classes_pred_100 = np.array(classes_pred_100)
		novel_classes = ['bottle ', 'bus ', 'couch ', 'microwave ', 'pizza ', 'racket ', 'suitcase ', 'zebra ']
		avg_f1 = 0.
		print('100 samples')
		for nc_i in range(len(novel_classes)):
			f1_score = sklearn.metrics.f1_score(classes_gt[:,nc_i], classes_pred_100[:,nc_i])
			avg_f1 += f1_score
			print(novel_classes[nc_i],' f1_score ', f1_score)
		avg_f1 = avg_f1 / 8.
		print('Avg F1 100 ', avg_f1)
