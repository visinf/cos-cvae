...
from modules.textmodules import TextDecoder, GaussPrior, TextEncoderMask, TextEncoderObj
...
def load_dataset(data_path,vocab_path,coco_class,maxlength):
	...
	image_data_path = str(data_path)+'/coco_train_2014_adaptive_withclasses.h5'
	text_data_path = str(data_path)+'/annotations/captions_train2014.json'
	coco_train = COCO_Dataset(text_data_path,image_data_path,vocab_path,coco_class,None,False,maxlength)
	...
	coco_test_idxs_files = [str(data_path)+'/coco_val_mRNN.txt',str(data_path)+'/coco_test_mRNN.txt']
	coco_train_val = COCO_Dataset(text_data_path,image_data_path,vocab_path,coco_class,coco_test_idxs_files,False,maxlength)
	return torch.utils.data.ConcatDataset([coco_train,coco_train_val])
...
if __name__== "__main__":
	...
	coco_dataloader = torch.utils.data.DataLoader(coco_dataset, batch_size=batch_size, shuffle=True, num_workers=4, drop_last=True)		
	...
	optimizer = optim.SGD( itertools.chain(texEncMask.parameters(), texEncObj.parameters(), texDec.parameters(), g_prior.parameters() ), 
		lr=0.015, momentum=0.9, weight_decay=0.001)#, glow_text_cond.parameters()
	...
	while iteration <max_iterations:
		...
		if epoch == 0:
			...
			kl_rate = 1.0
		train_bar = tqdm(coco_dataloader)
		for data in train_bar:
			...
			if not init_epoch:
				...
				for j in range(batch_size):
					...
					if im_idx[j] in coco_idx_nn_map:
						hash_map_idx = coco_idx_nn_map[im_idx[j]][nn_mask_idx]
			...
			if not init_epoch:
				bottom_up_features_nn,bottom_up_classes_nn,seq_nn,mask_seq_nn, _, seq_lengths_nn, sort_order = get_properseq(bottom_up_features_nn, bottom_up_classes_nn, seq_nn.cpu().numpy(), mask_seq_nn.cpu().numpy(), None, seq_lengths_nn.cpu().numpy())

				mask_q_means_nn, mask_q_logs_nn, mask_q_z_nn, _ = texEncMask(mask_seq_nn[:,1:].clone().cuda(),(seq_lengths_nn - 1).cuda())

				list_v_k_nn = bottom_up_features_nn.float() 
				list_v_k_nn = list_v_k_nn[:,:,:meanimfeats_size]

				kl_loss_nn =  g_prior(obj_enc=list_v_k_nn.cuda(), x=seq_nn.cuda(), p_z_t_1=None, hiddens=None, 
					q_means_mask=mask_q_means_nn, q_logs_mask=mask_q_logs_nn, q_z_mask=mask_q_z_nn, 
					q_means_obj=None, q_logs_obj=None, q_z_obj=None, train=True, reverse=False)

				logp, alphas, _ = texDec(seq_nn.long().cuda(),None,mask_q_z_nn.cuda(),list_v_k_nn.cuda(),(seq_lengths_nn-1).cuda(),g_prior)
