...
class COCO_Dataset(Dataset):
	...
	def __init__(self, text_data_path,image_data_path, vocab_path,coco_class,coco_test_idxs_files,test=False,maxlength=20):
		"""
		Args:
		...
		if 'val' in self.image_data_path and not test:
			imkeyfile_ = open(str(coco_test_idxs_files[0]),'r').read().split('\n')[:4000]
			imkeylist = []
			...
			imkeyfile_ = open(str(coco_test_idxs_files[1]),'r').read().split('\n')[:1000]
			imkeylist = []
			for images in imkeyfile_:
				imagekey = os.path.splitext(os.path.basename(images))[0]
				imagekey  = int(imagekey.split('_')[-1])
		...
		if test:
			test_idx = [int(np.where(self.imagefeatures["image_id"]==_key)[0]) for _key in imkeylist]
			for k in self.imagefeatures_h5.keys():
				self.imagefeatures[k] = self.imagefeatures[k][test_idx]
	...
	def __getitem__(self, idx):
		...
		return captions, x_m_caps, x_o_caps, bottom_up_features, bottom_up_classes, np.array(caption_length), image_idx
